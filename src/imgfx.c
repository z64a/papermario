...
#define RM_IMGFX_A1 \
    ...
    ( AA_EN | CVG_DST_FULL | CVG_X_ALPHA | ZMODE_OPA )

#define RM_IMGFX_A2 \
    GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_A_MEM) | \
    ( AA_EN | CVG_DST_FULL | CVG_X_ALPHA | ZMODE_OPA )

#define RM_IMGFX_B1 ((RM_ZB_CLD_SURF(1)) & ~Z_CMP)
//    GBL_c1(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA) | \
//    ( FORCE_BL | IM_RD | CVG_DST_SAVE | ZMODE_XLU )

#define RM_IMGFX_B2 ((RM_ZB_CLD_SURF(2)) & ~Z_CMP)
//    GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA) | \
//    ( FORCE_BL | IM_RD | CVG_DST_SAVE | ZMODE_XLU )

ImgFXRenderMode ImgFXRenderModes[] = {
    [IMGFX_RENDER_DEFAULT]               { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_MULTIPLY_RGB]          { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_MULTIPLY_ALPHA]        { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_MULTIPLY_RGBA]         { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_MODULATE_PRIM_RGB]     { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_MODULATE_PRIM_RGBA]    { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_MULTIPLY_SHADE_RGB]    { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_MULTIPLY_SHADE_ALPHA]  { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_MULTIPLY_SHADE_RGBA]   { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_MODULATE_SHADE_RGB]    { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_MODULATE_SHADE_RGBA]   { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_ANIM]                  { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_HOLOGRAM]              { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_COLOR_FILL]            { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_OVERLAY_RGB]           { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
    [IMGFX_RENDER_OVERLAY_RGBA]          { RM_IMGFX_B1, RM_IMGFX_B2, IMGFX_RENDER_NO_OVERRIDE },
    [IMGFX_RENDER_UNUSED]                { RM_IMGFX_A1, RM_IMGFX_A2, 0 },
};
...
s32 imgfx_appendGfx_component(s32 idx, ImgFXTexture* ifxImg, u32 flagBits, Matrix4f mtx) {
    ImgFXState* state = &(*ImgFXInstances)[idx];
    s32 ret = IMGFX_RENDER_RESULT_OK;
    ...
    if (state->flags & IMGFX_FLAG_ANIM_DONE) {
        ...
        imgfx_cache_instance_data(state);
        ret = IMGFX_RENDER_RESULT_DONE;
    } else if (state->flags & IMGFX_FLAG_HOLDING) {
        ret = IMGFX_RENDER_RESULT_HOLDING;
    } else if (state->flags & IMGFX_FLAG_FORCE_CLEAR) {
        state->lastAnimCmd = IMGFX_CLEAR;
        state->lastColorCmd = IMGFX_CLEAR;
        state->meshType = IMGFX_MESH_DEFAULT;
        state->renderType = IMGFX_RENDER_DEFAULT;
...
void imgfx_appendGfx_mesh(ImgFXState* state, Matrix4f mtx) {
    ...
    if (!(state->flags & IMGFX_FLAG_SKIP_GFX_SETUP)) {
        ...
        if (state->flags & IMGFX_FLAG_SKIP_ZBUF) {
...
void imgfx_appendGfx_mesh_basic(ImgFXState* state, Matrix4f mtx) {
    ...
    while (true) {
        ...
        s32 useShadingFlags = IMGFX_FLAG_SPRITE_SHADING | IMGFX_FLAG_AS_SPRITE;
        s32 alpha;
        s32 alpha2;
